...
globi_eat_s %>% 
  ...
  filter (nspecies==1) %>%
  bind_cols(s_eatLCA_l="species") %>% 
  select(qtax_species,s_n_eat,s_eatLCA_v,s_eatLCA_l) -> s_speciesdata2 #39
...
globi_eat_s %>% 
  ...
  bind_cols(s_eatLCA_l="genus") %>% 
  select(qtax_species,s_n_eat,s_eatLCA_v,s_eatLCA_l) -> s_genusdata2 #5
...
globi_eat_s %>% 
  ...
  bind_cols(s_eatLCA_l="family") %>% 
  select(qtax_species,s_n_eat,s_eatLCA_v,s_eatLCA_l) -> s_familydata2 #3
...
globi_eat_s %>% 
  ...
  bind_cols(s_eatLCA_l="order") %>% 
  select(qtax_species,s_n_eat,s_eatLCA_v,s_eatLCA_l) -> s_orderdata2 #4
...
globi_eat_s %>% 
  ...
  bind_cols(s_eatLCA_l="class") %>% 
  select(qtax_species,s_n_eat,s_eatLCA_v,s_eatLCA_l) -> s_classdata2 #12
...
globi_eat_s %>% 
  ...
  filter (nspecies !=1 & ngenus!=1 & nfam !=1 & norder!=1 & nclass!=1 & 
            nphylum==1) %>%
  bind_cols(s_eatLCA_l="phylum") %>% 
  select(qtax_species,s_n_eat,s_eatLCA_v,s_eatLCA_l) -> s_phylumdata2 #7
...
globi_eat_s %>% 
  ...
  bind_cols(s_eatLCA_l="superkingdom") %>% 
  select(qtax_species,s_n_eat,s_eatLCA_v,s_eatLCA_l) -> 
...
globi_eat_s %>% 
  ...
  filter (nspecies !=1 & ngenus!=1 & nfam !=1 & norder!=1 & nclass!=1 & 
            nphylum!=1 & nkingdom>1) %>%
  bind_cols(s_eatLCA_l="Bac + Euk",s_eatLCA_v=">superkingdom") %>% 
  select(qtax_species,s_n_eat,s_eatLCA_v,s_eatLCA_l) -> s_nolcadata2 #23
...
globi_eat_s %>% 
  ...
  bind_cols(s_eatCTV_l="species",s_eatCTV_v="Vulpes vulpes") %>%
  select(qtax_species,s_eatCTV_v,s_eatCTV_l) -> s_vspecies2 #0
...
globi_eat_s %>% 
  ...
  bind_cols(s_eatCTV_l="genus",s_eatCTV_v="Vulpes") %>%
  select(qtax_species,s_eatCTV_v,s_eatCTV_l) -> s_vgenus2 #13
...
globi_eat_s %>% 
  ...
  filter (nvulpesspecies %in% c(NA,0) & nvulpesgenus %in% c(NA,0) & 
            nvulpesfam>=1) %>%
  bind_cols(s_eatCTV_l="family",s_eatCTV_v="Canidae") %>%
  select(qtax_species,s_eatCTV_v,s_eatCTV_l) -> s_vfam2 #1
...
globi_eat_s %>% 
  ...
  bind_cols(s_eatCTV_l="order",s_eatCTV_v="Carnivora") %>%
  select(qtax_species,s_eatCTV_v,s_eatCTV_l) -> s_vorder2 #12
...
globi_eat_s %>% 
  ...
  filter (nvulpesspecies %in% c(NA,0) & nvulpesgenus %in% c(NA,0) & 
            ...
            nvulpesclass>=1) %>%
  bind_cols(s_eatCTV_l="class",s_eatCTV_v="Mammalia") %>%
  select(qtax_species,s_eatCTV_v,s_eatCTV_l) -> s_vclass2 #43
...
globi_eat_s %>% 
  ...
  bind_cols(s_eatCTV_l="phylum",s_eatCTV_v="Chordata") %>%
  select(qtax_species,s_eatCTV_v,s_eatCTV_l) -> s_vphylum2 #45
...
globi_eat_s %>% 
  ...
  filter (nvulpesspecies %in% c(NA,0) & nvulpesgenus %in% c(NA,0) & 
            ...
            nvulpeskingdom>=1) %>%
  bind_cols(s_eatCTV_l="superkingdom",s_eatCTV_v="Eukaryota") %>%
  select(qtax_species,s_eatCTV_v,s_eatCTV_l) -> s_vkingdom2 #105
...
globi_eat_s %>% 
  ...
  bind_cols(s_eatCTV_l="No level ctv",s_eatCTV_v="No") %>%
